...
import type { Department, Direction } from "../data/platform-mock";
...
const DEFAULT_SCOPE: ScopeFilter = { department: "all", direction: "all" };

export function Phase1EmailPreview({ emailType, enabled, sections, scope = DEFAULT_SCOPE }: Props) {
  if (!enabled) {
    return <PausedState emailType={emailType} />;
  }

  if (emailType === "daily") {
    const base: DailyDigestData = pickDailyFixture(scope.department);
  ...
  if (emailType === "weekly") {
    const base: WeeklyData = pickWeeklyFixture(scope.department, scope.direction);
  ...
  if (emailType === "monthly") {
    const base: MonthlyData = pickMonthlyFixture(scope.department, scope.direction);
...
function pickDailyFixture(dept: Department): DailyDigestData {
  if (dept === "sales") return DAILY_NORMAL_SALES;
  if (dept === "service") return DAILY_NORMAL_SERVICE;
  return DAILY_NORMAL_BOTH;
}

function pickWeeklyFixture(dept: Department, dir: Direction): WeeklyData {
  const department = dept === "all" ? "sales" : dept;
  const direction = dir === "all" ? "ib" : dir;
  if (department === "sales" && direction === "ib") return WEEKLY_SALES_IB_ONLY;
  if (department === "sales" && direction === "ob") return WEEKLY_SALES_OB_ONLY;
  if (department === "service" && direction === "ib") return WEEKLY_SERVICE_IB_ONLY;
  return WEEKLY_SERVICE_OB_ONLY;
}

// v4: Monthly now ships 4 agent-specific variants (mirrors Weekly).
function pickMonthlyFixture(dept: Department, dir: Direction): MonthlyData {
  const department = dept === "all" ? "sales" : dept;
  const direction = dir === "all" ? "ib" : dir;
  if (department === "sales" && direction === "ib") return MONTHLY_SALES_IB_ONLY;
  if (department === "sales" && direction === "ob") return MONTHLY_SALES_OB_ONLY;
  if (department === "service" && direction === "ib") return MONTHLY_SERVICE_IB_ONLY;
  return MONTHLY_SERVICE_OB_ONLY;
}
...
function applyMonthlyToggles(
  ...
  // v5: value_estimate_sidebar removed from MonthlyData entirely;
  //     customer_centric.lapsed_reengaged → routed_calls.
  return {
    ...base,
    kpi_grid: on(sections, "kpi_grid") ? base.kpi_grid : [],
    customer_centric: on(sections, "customer_centric")
      ? base.customer_centric
      : { unique: 0, pct_new: 0, pct_returning: 0, avg_touches: 0, routed_calls: 0 },
...
function applyEOCToggles(base: EOCData, sections: SectionMap): EOCData {
  // v4: funnel renamed to conversion_funnel (Reached leads → Contacted → Appointments → ABR%).
  // `conversion_by_vehicle_or_service` and `recommendations` removed from the schema per §9.4.
  return {
    ...base,
    conversion_funnel: on(sections, "funnel")
      ? base.conversion_funnel
      : { reached_leads: 0, contacted: 0, appointments: 0, abr_pct: 0 },
