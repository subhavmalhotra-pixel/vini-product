...
import {
  Phase1EmailPreview,
  type ScopeFilter,
} from "../components/Phase1EmailPreview";
...
export function SubscriptionsEdit() {
  ...
  const [scopeByType, setScopeByType] = useState<Record<EmailType, ScopeState>>(() => ({
    daily: { ...DEFAULT_SCOPE },
    weekly: { ...DEFAULT_SCOPE },
    monthly: { ...DEFAULT_SCOPE },
    eoc: { ...DEFAULT_SCOPE },
  }));
  ...
  const scope = scopeByType[activeTab];
  const directionApplies = DIRECTION_APPLIES_TO[activeTab] ?? false;
  const previewScope: ScopeFilter = {
    department: scope.department,
    direction: directionApplies ? scope.direction : "all",
  };
  const updateScope = (next: Partial<ScopeState>) =>
    setScopeByType((prev) => ({
      ...prev,
    ...
    <div className="px-4 sm:px-6 md:px-8 py-5 sm:py-6 md:py-8 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-5 lg:gap-6">
        ...
        <div className="space-y-4 min-w-0">
          ...
          <ScopeFiltersCard
            scope={scope}
            directionApplies={directionApplies}
...
function ScopeFiltersCard({
  ...
  const departmentOptions: Array<{ key: Department; label: string }> = [
    { key: "all", label: "All" },
    { key: "sales", label: "Sales" },
    { key: "service", label: "Service" },
  ];
  const directionOptions: Array<{ key: Direction; label: string }> = [
    { key: "all", label: "Both" },
    { key: "ib", label: "Inbound" },
  ...
  return (
    <div className="bg-surface-card border border-border-subtle rounded-xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-border-subtle">
        <h3 className="text-sm font-semibold text-text-primary">Scope filters</h3>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {departmentOptions.map((opt) => {
              const active = scope.department === opt.key;
              return (
                <button
                  key={opt.key}
                  type="button"
                  onClick={() => onChange({ department: opt.key })}
                  className={`min-h-[32px] px-3 rounded-md text-xs font-semibold transition-colors border ${
                    active
                      ? "bg-brand-primary text-white border-brand-primary"
                      : "bg-surface-card text-text-primary border-border-subtle hover:bg-surface-background"
                  }`}
                >
                  {opt.label}
                </button>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {directionOptions.map((opt) => {
                const active = scope.direction === opt.key;
                return (
                  <button
                    key={opt.key}
                    type="button"
            ...
            <p className="mt-1.5 text-[11px] text-text-muted">
              Inbound = calls/chats initiated by customer. Outbound = agent-initiated campaigns.
            </p>
          </div>
        )}
      </div>
