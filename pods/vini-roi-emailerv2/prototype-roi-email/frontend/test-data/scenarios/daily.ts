...
export const DAILY_NORMAL_BOTH: DailyDigestData = {
  ...
  scenario_notes:
    "v3: GM view restored. Hero KPIs in inbound = Appointments + Unique leads. Sub-metrics include Avg response time (sales) AND Transfer rate (service) since both depts are in scope. Action items use a single bottom 'Review action items' CTA — no per-line review buttons.",
  email_type: "daily",
  send_decision: "send",
  dealer: HONDA_DTLA,
  ...
  inbound: {
    // v3: kpi_cards drives BOTH hero (first 2 large) AND sub-metrics (rest small).
    // Hero KPIs: Appointments + Unique leads.
    // Sub-metrics for GM view: Avg response time (sales-relevant) + Transfer rate (service-relevant).
    kpi_cards: [
      ...
      { label: "Avg response time", primary_value: 47, primary_unit: "s", subtitle: "1m 02s MTD" },
      { label: "Transfer rate", primary_value: 12, primary_unit: "%", subtitle: "7 transfers · 11% MTD" },
    ],
    activity: {
...
// ============================================================================
// 2. NORMAL — Sales-only subscription (v3 — hero KPIs + sub-metric)
// ============================================================================
export const DAILY_NORMAL_SALES: DailyDigestData = {
  ...
  scenario_notes:
    "v3: Single bottom 'Review action items' CTA. Inbound hero KPIs = Appointments + Unique leads (large). Sub-metric = Avg response time (sales-relevant). BDC-hours equivalent suppressed.",
  ...
  inbound: {
    // v3: Hero KPIs (Appointments + Unique leads) + sales sub-metric (Avg response time).
    kpi_cards: [
      { label: "Appointments", primary_value: 14, subtitle: "Yesterday · 142 MTD" },
      { label: "Unique leads", primary_value: 58, subtitle: "Yesterday · 612 MTD" },
      { label: "Avg response time", primary_value: 47, primary_unit: "s", subtitle: "1m 02s MTD" },
...
export const DAILY_NORMAL_SERVICE: DailyDigestData = {
  ...
  scenario_notes:
    "v3: Hero KPIs = Appointments + Unique leads. Sub-metric = Transfer rate (service-relevant — measures how often Vini hands callers to a human advisor). Inbound channel split suppressed (Service voice-dominant). Single bottom 'Review action items' CTA. BDC-hours suppressed.",
  email_type: "daily",
  send_decision: "send",
  dealer: TOYOTA_TAMPA,
  ...
  inbound: {
    // v3: Hero KPIs (Appointments + Unique leads) + service sub-metric (Transfer rate).
    kpi_cards: [
      ...
      { label: "Transfer rate", primary_value: 15, primary_unit: "%", subtitle: "5 transfers · 11% MTD" },
    ],
    activity: {
...
export const DAILY_ZERO_INBOUND_CALLS: DailyDigestData = {
  ...
  scenario_notes:
    "v3: Calls = 0, SMS/Chat normal. Email renders as a regular Sales digest with a 0 in the call column. Hero KPIs + Avg response time sub-metric still rendered consistently. NO 'verify telephony' banner — that alert fires to internal Slack only.",
  channel_split: { call: 0, sms: 51, chat: 27 },
  conversations_total: 78,
  inbound: {
    // v3: same hero + sub-metric structure as the parent Sales scenario,
    // with values reflecting the call-channel outage.
    kpi_cards: [
      { label: "Appointments", primary_value: 11, subtitle: "Yesterday · 138 MTD" },
      { label: "Unique leads", primary_value: 30, subtitle: "Yesterday · 584 MTD" },
      { label: "Avg response time", primary_value: 52, primary_unit: "s", subtitle: "1m 02s MTD" },
    ],
    activity: {
...
export const DAILY_YESTERDAY_ZERO_MTD_POSITIVE: DailyDigestData = {
  ...
  action_required: [],
  inbound: {
    // v3: Hero KPIs always rendered (zero value shown in neutral gray). Sub-metric
    // (Avg response time) marked unavailable for the day — MTD context preserved.
    kpi_cards: [
      ...
      { label: "Unique leads", primary_value: 0, subtitle: "Yesterday · 612 MTD" },
      {
        ...
        subtitle: "1m 02s MTD",
        unavailable: true,
        unavailable_reason: "no_activity",
      },
    ],
    activity: {
...
export const DAILY_DAY_1_ROOFTOP: DailyDigestData = {
  ...
  inbound: {
    // v3: Day-1 hero replaces the standard 2-card hero with a celebration/onboarding
    // tile (handled in the email component). So for Day 1–7, we keep only one KPI card
    // for engineer-side reference: leads engaged. The render skips the regular hero
    // grid entirely when show_onboarding_banner is true.
    kpi_cards: [
...
export const DAILY_SMALL_DENOMINATOR: DailyDigestData = {
  ...
  inbound: {
    // v3: Service low-volume — hero KPIs show raw counts, Transfer rate marked unavailable
    // (tiny sample is statistically noisy) while MTD context is preserved.
    kpi_cards: [
      ...
      { label: "Unique leads", primary_value: 3, subtitle: "Yesterday · 412 MTD" },
      {
        ...
        subtitle: "11% MTD",
        unavailable: true,
        unavailable_reason: "small_sample",
      },
    ],
    activity: {
...
// v3: GM (all-agents) view restored at the top of the list. Surfaces both
// Avg response time AND Transfer rate as sub-metrics since both depts are in scope.
