...
Renders 20 email scenarios (Daily / Weekly / Monthly / End-of-Campaign) including every edge case from PRD §9. Sidebar scenario switcher lets you click through each.
...
### 3. Eval runner — verify the harness against PRD §4
...
| 20 mock email scenarios (all edge cases per PRD §9) | Recipient management persistence |
...
### `ai-harness/` (Python — implements PRD §4 exactly)
...
3. **Cost ceilings live in `config.py`.** Adjust there if PRD §4 changes — never inline.
