...
type Props = {
  ...
  /** Opens the Mark-as-incorrect reason picker · v3.1. */
  onMarkIncorrect?: (item: ActionItem) => void;
};
        ...
        <div className="animate-fade-in-up border-t border-border-subtle bg-surface-subtle/60 px-5 pb-4 pt-3">
          <div className="grid grid-cols-[1fr_auto] gap-4">
            ...
            <div className="flex flex-shrink-0 flex-col items-end gap-1.5">
              ...
              {/* Mark-as-incorrect tertiary affordance · v3.1.
                  Intentionally low visual weight so closers don't lazy-use
                  it instead of writing a real resolution note. */}
              {onMarkIncorrect ? (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  title="Wrongly generated by AI · removes from queue without counting as closure"
                  className="text-[11px] font-medium text-text-tertiary transition-colors duration-150 hover:text-status-warning"
                >
                  ⚠ Mark as incorrect
                </button>
              ) : null}
