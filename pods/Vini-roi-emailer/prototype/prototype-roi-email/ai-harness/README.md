...
Implements PRD §4 of `pods/vini-reporting/prd-roi-emailer.md`.
...
7. **Short-output check** — if output < 100 chars, fallback engages (per PRD §4).
...
All ceilings are in `config.py`. They come directly from PRD §4 and the kill criteria in PRD §7:

| Setting | Value | Source |
|---|---|---|
| Model | claude-haiku-4-5-20251001 | PRD §4 |
| Max output tokens | 600 | PRD §4 |
| Max input tokens | 4000 | PRD §4 |
| Request timeout | 10s | PRD §4 |
| Per-request cost ceiling | $0.02 | PRD §4 |
| Per-week-per-dealer | $0.20 | PRD §4 |
| Per-day-per-dealer | $1.00 | PRD §4 |
| Rolling-24h kill threshold | $0.05 avg | PRD §7 KC4 |
